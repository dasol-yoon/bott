...
def run_one_trial(
    ...
    if os.path.exists(results_dir + f"trial_{trial}.pt") and not force_restart:
        ...
        if algo=='EICF':
            ...
            reductionLoss = loss_func(y_simu=y_reduction*problem.scaling_factor.to(device),
                                        y_true=reduction_true*problem.scaling_factor.to(device),
    ...
    for iter in range(n_init_evals, max_iter): # Feel like we should match the value with len(X)
        ...
        if algo=='EICF':
            ...
            reductionLoss = loss_func(y_simu=y_reduction.unsqueeze(0)*problem.scaling_factor.to(device),
                                        y_true=reduction_true*problem.scaling_factor.to(device),
