...
def run_one_trial(
    ...
    if os.path.exists(results_dir + f"trial_{trial}.pt") and not force_restart:
        ...
        ground_truth_original = res['ground_truth_original']
        ...
        X=draw_sobol_samples(bounds=problem.bounds.to(device=device),n=n_init_evals,q=1,seed=trial).squeeze(-2).to(dtype=dtype) # X = [n_init, problem.dim], note that X is by default on cpu because problem.bounds is also on cpu
        ...
        if algo=='EICF':
            ...
            logging.info(f'(For EICF) Initial y_reduction (patch values) for {y_reduction.shape[0]} inputs: {y_reduction}')
            ...
            logging.info(f'(For EICF) reductionLoss (patch SSE) for {reductionLoss.shape[0]} inputs: {reductionLoss}')
    ...
    for iter in range(n_init_evals, max_iter): # Feel like we should match the value with len(X)
        ...
        logging.info(f'Using max to scale image to (0,1) then scale up to overall_scaling_factor')
        image_temp = (image_temp / image_temp.max())*problem.overall_scaling_factor #3/20/2026 changed to max to scale image to (0,1) then scale up to overall_scaling_factor
        ...
        logger.info(f"Acqf function sampling time: {time_sample_end - time_sample_start:.3f} sec")
        ...
        logger.info(f"New objective function value (-pixelSSE): {-1*new_Loss.cpu().numpy()}")
